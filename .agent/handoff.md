...
- Current uncommitted work adds `tests/dj-analyzer.test.js` coverage for long podcast range sampling returning an empty beatmap when all ranged audio responses decode to no audio frames.
...
- `node --test tests/dj-analyzer.test.js`: passed, 8 tests.
...
- `node --test --experimental-test-coverage tests/*.test.js`: passed, 169 tests; all-files line coverage 92.91%, branch coverage 65.07%, function coverage 89.65%; `server.js` line coverage 89.82%; `lib/update-utils.js` line coverage 100.00%; `dj-analyzer.js` line coverage 70.02%.
...
- `dj-analyzer.js` pure beat-map generation is covered for empty and pulse-grid paths, and wrapper failure/full-stream-empty/empty-intro/empty-range paths have first-pass coverage; range-sampled success and non-empty decoded audio success paths remain largely untested.
