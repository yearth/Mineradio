...
- Worktree: server shell finalization is in progress; latest verified slice extracts update runtime adapter wiring into `server/composition/update-runtime-adapters.ts`.
...
- Latest completed slice: update runtime adapter wiring moved from `server.js` into `server/composition/update-runtime-adapters.ts`; `server.js` now creates `updateAdapters` and destructures the legacy update dependencies (`updateDownloadJobs`, `publicUpdateJob`, `fetchLatestUpdateInfo`, `localUpdateFallback`, `startUpdateDownloadJob`, `startUpdatePatchJob`, `moveInvalidUpdateFile`) instead of owning the full update wrapper block.
- Latest verification: initial RED `npm run build:ts && node --test tests/update-runtime-adapters.test.js` failed with missing `../server-dist/server/composition/update-runtime-adapters`; first focused integration caught a real regression where `fetch` was frozen at adapter creation time, breaking `update-routes` tests and test-time `global.fetch` replacement; fixed by switching adapter wiring to `getFetch()` lazy lookup. Focused/static `npm run build:ts && node --test tests/update-runtime-adapters.test.js tests/update-controller.test.js tests/update-routes.test.js tests/ops-route-composition.test.js tests/server-test-runtime.test.js && node --check server.js && npm run typecheck && git diff --check` passed, 42 tests. `npm test && npm run coverage` passed, 567 tests; all files line coverage `100.00%`, including `server-dist/server/composition/update-runtime-adapters.js` and `server.js`.
- Server shell size after this slice: `server.js` is 780 lines, down from 1029 before extracting update adapter wiring.
- Next recommended slice: extract QQ/Netease/provider request adapters or app/bootstrap/TLS composition, preserving lazy runtime getters such as `getFetch`, current cookies, and Netease API overrides. Do not mark the extremely-thin-shim goal complete until `server.js` is only a compatibility entry that delegates to TS entry/composition.
