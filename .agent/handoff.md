...
- Worktree: server shell finalization is in progress; latest verified slice extracts root route dependency assembly into `server/root-dependencies.ts`.
...
- Next recommended slice: final `server.js` bootstrap cleanup. Consider extracting the remaining root route dependency factory declarations into typed domain-specific factory modules only if lazy closure semantics for `fetch`, `currentUserCookie`, QQ cookie state, and `__test.setNeteaseApi` overrides can be preserved with tests.
