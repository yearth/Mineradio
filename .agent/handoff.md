...
- Worktree: server shell finalization is in progress; latest verified slice extracts root route runtime factory assembly into `server/root-runtime-factories.ts`.
...
- Next recommended slice: final review of `server.js` for remaining bootstrap-only boundaries. Avoid moving the remaining provider wrapper functions unless new tests explicitly protect `currentUserCookie`, QQ cookie state, and Netease API override behavior.
