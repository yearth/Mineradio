...
- Worktree: renderer HTML slimming is in progress; latest local slice moves the large inline renderer script from `public/index.html` into `public/renderer/app.js`. QA review is in progress.
- Current phase: Electron renderer modularization after completing the `server.js` thin-shim goal.
- Active goal: safely externalize the large inline renderer JavaScript into `public/renderer/app.js`, preserving vendor script order, the head preload class script, global function compatibility, Electron/preload behavior, static resource serving, app packaging, UI behavior, and backend test stability.
- Latest completed renderer slice: CSS was externalized from `public/index.html` into `public/styles/app.css` and committed as `7b34391 refactor: extract renderer stylesheet`; `index.html` retained a stylesheet link and the macOS polish CSS remained available in the bundle.
- Current uncommitted renderer slice: the large inline `<script>` starting with `'use strict'` was moved to `public/renderer/app.js`; `public/index.html` now keeps vendor scripts, the small mode-preload inline script, `styles/app.css`, and a trailing `<script src="renderer/app.js"></script>`.
- Current renderer verification: `node --check public/renderer/app.js` passed. Local server check on `PORT=3137` returned `/` 200, `/styles/app.css` 200, and `/renderer/app.js` 200; HTML includes `styles/app.css` and `renderer/app.js`, no longer includes the large inline Global State block, and external JS contains the Global State block. `npm run build:ts`, `node --check server.js`, and `npm test` passed with 578 tests. `npm run build:mac` passed; `dist/mac-arm64/Mineradio.app/Contents/Resources/app/public/renderer/app.js` exists. QA subagent review is in progress.
- Current renderer size: `public/index.html` is 849 lines, `public/styles/app.css` is 2172 lines, and `public/renderer/app.js` is 24205 lines.
- Next recommended renderer slice after QA/commit: split `public/renderer/app.js` by safe boot/runtime boundaries, starting with low-risk constants/state/API helper extraction rather than changing UI behavior.
...
- Latest completed committed slice: QQ route adapter wiring moved from `server.js` into `server/composition/qq-route-adapters.ts`; `server.js` now creates `qqRouteAdapters` and destructures the legacy QQ handler names (`handleQQUserPlaylists`, `handleQQPlaylistTracks`, `handleQQArtistDetail`, `handleQQSearch`, `handleQQSongUrl`, `handleQQSongComments`, `handleQQLyric`) instead of owning QQ route-level wrapper logic.
- Latest committed verification: initial RED `npm run build:ts && node --test tests/qq-route-adapters.test.js` failed with missing `../server-dist/server/composition/qq-route-adapters`; focused/static `npm run build:ts && node --test tests/qq-route-adapters.test.js tests/qq-request-adapters.test.js tests/qq-controller.test.js tests/qq-composition.test.js tests/qq-orchestration-service.test.js tests/music-routes.test.js tests/simple-route-composition.test.js && node --check server.js && npm run typecheck && git diff --check` passed, 181 tests. `npm test && npm run coverage` passed, 574 tests; all files line coverage `100.00%`, including `server-dist/server/composition/qq-route-adapters.js` and `server.js`.
- Latest completed committed slice: Netease route adapter wiring moved from `server.js` into `server/composition/netease-route-adapters.ts`; `server.js` now creates `neteaseRouteAdapters`, shares one lazy `getNeteaseApi()` with root route factories, and destructures the legacy Netease handler names (`getLoginInfo`, `requireLogin`, `handleSearch`, `handleDiscoverHome`, `fetchMyPodcastItems`, `handleSongUrl`) instead of owning Netease route-level wrapper logic.
- Latest committed verification: initial RED `npm run build:ts && node --test tests/netease-route-adapters.test.js` failed with missing `../server-dist/server/composition/netease-route-adapters`; focused/static `npm run build:ts && node --test tests/netease-route-adapters.test.js tests/netease-orchestration-service.test.js tests/netease-auth-controller.test.js tests/netease-library-controller.test.js tests/netease-media-controller.test.js tests/netease-media-composition.test.js tests/podcast-controller.test.js tests/music-routes.test.js tests/root-runtime-factories.test.js && node --check server.js && npm run typecheck && git diff --check` passed, 212 tests. `npm test && npm run coverage` passed, 577 tests; all files line coverage `100.00%`, including `server-dist/server/composition/netease-route-adapters.js` and `server.js`. QA subagent review: `PASS`; read-only QA verified lazy `getNeteaseApi()`, current cookie/saveCookie wiring, `requireLogin` 401 `sendJSON`, root factory lazy API behavior, `NODE_ENV=test` export compatibility, and focused/coverage validation.
- Current uncommitted slice: root server composition moved from `server.js` into `server/server-entry.ts`; `server.js` now requires `./server-dist/server/server-entry`, calls `createMineradioServerEntry({ rootDir: __dirname, env: process.env, getFetch: () => fetch })`, exports `entry.server`, and attaches `entry.testRuntime` under `NODE_ENV === 'test'`.
- Current verification: initial RED `npm run build:ts && node --test tests/server-entry.test.js tests/server-test-runtime.test.js` failed with missing `../server-dist/server/server-entry`; GREEN same command passed, 5 tests. Focused/static `npm run build:ts && node --test tests/server-entry.test.js tests/server-test-runtime.test.js tests/server-bootstrap.test.js tests/server-router.test.js tests/project-structure.test.js tests/server-helpers.test.js tests/music-routes.test.js tests/update-routes.test.js tests/beatmap-cache-routes.test.js tests/weather-mood.test.js && node --check server.js && npm run typecheck && git diff --check` passed, 195 tests. `npm test` passed, 578 tests. `npm run coverage` passed, 578 tests; all files line coverage `100.00%`, including `server.js` and `server-dist/server/server-entry.js`. Final static `npm run build:ts && node --check server.js && npm run typecheck && git diff --check` passed. QA subagent review: `PASS`; read-only QA verified Electron still requires root `server.js`, the shim exports `entry.server` and attaches `entry.testRuntime`, `server-entry.ts` preserves lazy `getFetch`, lazy Netease provider/runtime overrides, legacy `__test` bindings, focused/full/coverage validation, and noted the expected `npm run build:ts` prerequisite for ignored `server-dist/`.
- Server shell size after current slice: `server.js` is 14 lines, down from 527 before extracting root server entry composition.
- Next recommended slice after QA/commit: if QA passes, mark the active thin-shim goal complete. Follow-up refactors should target internal cleanliness of `server/server-entry.ts` only if desired, but the original `server.js` thin-shim goal should be considered achieved once committed and verified.
...
QQ route adapter extraction:

- Initial RED: `npm run build:ts && node --test tests/qq-route-adapters.test.js` failed with `Cannot find module '../server-dist/server/composition/qq-route-adapters'`.
- New composition module: `server/composition/qq-route-adapters.ts` exports `createQQRouteAdapters(...)`, which owns the previous QQ route-level wrappers for user playlists, playlist tracks, artist detail, search, song URL/vkey lookup, song comments, and lyric.
- Root wiring: `server.js` now creates `qqRouteAdapters` from the lower-level QQ request adapters (`getQQLoginInfo`, `qqGetJSON`, `qqMusicRequest`, `qqSmartboxSearch`, `qqSongDetail`, QQ cookie helpers) and destructures the same legacy handler names used by `createRootRouteRuntimeFactories(...)`.
- Compatibility covered by tests: adapter handler surface, service delegation, QQ song URL missing-mid payload, deterministic guid injection, cookie-derived uin/music/playback keys, vkey filename/songmid/songtype params, `comm.authst`, successful playable payload, and unavailable playback payload.
- Focused/static verification: `npm run build:ts && node --test tests/qq-route-adapters.test.js tests/qq-request-adapters.test.js tests/qq-controller.test.js tests/qq-composition.test.js tests/qq-orchestration-service.test.js tests/music-routes.test.js tests/simple-route-composition.test.js && node --check server.js && npm run typecheck && git diff --check` passed, 181 tests.
- Final full verification: `npm test && npm run coverage` passed; 574 tests passed; production-code line coverage `100.00%`, including `server.js` and `server-dist/server/composition/qq-route-adapters.js` at `100.00%`.
- QA subagent review: `PASS`. Read-only QA verified `server.js` now requires/creates `createQQRouteAdapters(...)`, destructures the seven expected QQ handlers, old root-level QQ wrapper functions are gone, playlists/tracks/artist/search/comments/lyric delegate to original orchestration services, QQ song URL missing/success/unavailable paths preserve guid/cookie/vkey/auth payload behavior, and `requestRuntime`/`__test.setRequestText` remain sourced from `qq-request-adapters`.
