...
- Worktree: server shell finalization is in progress; latest verified slice extracts Netease provider API proxy wiring into `server/runtime/netease-provider-runtime.ts`.
- Current phase: root `server.js` compatibility shell slimming, keeping `server.js` as the runtime dependency-injection boundary.
...
- Latest completed slice: Netease provider API proxy wiring moved from `server.js` into `server/runtime/netease-provider-runtime.ts`; `server.js` now creates `neteaseProvider`, keeps `neteaseProvider.runtime` as `neteaseApiRuntime` for `__test.setNeteaseApi`, and destructures the legacy API names from `neteaseProvider.api`.
- Latest verification: initial RED `npm run build:ts && node --test tests/netease-provider-runtime.test.js` failed with missing `../server-dist/server/runtime/netease-provider-runtime`; focused/static `npm run build:ts && node --test tests/netease-provider-runtime.test.js tests/netease-api-runtime.test.js tests/root-runtime-factories.test.js tests/music-routes.test.js tests/netease-auth-controller.test.js tests/netease-library-controller.test.js tests/netease-media-controller.test.js && node --check server.js && npm run typecheck && git diff --check` passed, 173 tests. `npm test && npm run coverage` passed, 562 tests; all files line coverage `100.00%`, including `server-dist/server/runtime/netease-provider-runtime.js` and `server.js`.
- Next recommended slice: extract the remaining session/runtime wiring from `server.js` only after adding tests that protect `currentUserCookie`, `currentQQCookie`, `saveCookie`, `saveQQCookie`, `__test.setNeteaseApi`, `setRequestText`, and `resetMusicRuntime` dynamic behavior. Do not touch UI or Electron/package entry points.
