...
- Worktree: server shell finalization is in progress; latest verified local slice moves root server composition into `server/server-entry.ts`, leaving `server.js` as a 14-line CommonJS shim. QA review is in progress.
