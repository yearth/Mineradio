...
- Worktree: server shell finalization is in progress; latest verified slice extracts root HTTP route dispatch into `server/root-dispatcher.ts`.
...
- Next recommended slice: extract the root dependency/context factory assembly from `server.js` into typed composition helpers without changing lazy per-request construction or the legacy `__test` override behavior.
