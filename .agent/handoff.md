...
- Worktree: server shell finalization is in progress; latest verified slice extracts cookie session facade wiring into `server/runtime/session-runtime.ts`.
...
- Latest completed slice: cookie session facade wiring moved from `server.js` into `server/runtime/session-runtime.ts`; `server.js` now creates `sessionRuntime` from the existing `cookieRuntime`, destructures the legacy `currentUserCookie`, `currentQQCookie`, `saveCookie`, and `saveQQCookie` function names, and calls `sessionRuntime.reset()` inside `__test.resetMusicRuntime`.
- Latest verification: initial RED `npm run build:ts && node --test tests/session-runtime.test.js` failed with missing `../server-dist/server/runtime/session-runtime`; focused/static `npm run build:ts && node --test tests/session-runtime.test.js tests/cookie-runtime.test.js tests/root-runtime-factories.test.js tests/server-test-runtime.test.js tests/music-routes.test.js && node --check server.js && npm run typecheck && git diff --check` passed, 152 tests. `npm test && npm run coverage` passed, 564 tests; all files line coverage `100.00%`, including `server-dist/server/runtime/session-runtime.js` and `server.js`.
- Next recommended slice: extract the remaining test runtime/reset wiring from `server.js` only after adding tests that protect `__test.setNeteaseApi`, `setRequestText`, `resetMusicRuntime`, and update-runtime hook delegation. Do not touch UI or Electron/package entry points.
