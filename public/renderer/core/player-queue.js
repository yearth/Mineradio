...
function defaultCloneSong(song) {
  return Object.assign({}, song || {});
}

function createPlayerQueueHelpers(deps) {
  deps = deps || {};
  var resolveSongProviderKey = deps.songProviderKey || defaultSongProviderKey;
  var resolveCloneSong = deps.cloneSong || defaultCloneSong;

  function songProviderKey(song) {
    return resolveSongProviderKey(song);
  }

  function queueItemKey(song) {
    var providerKey = songProviderKey(song);
    if (!song) return '';
    if (providerKey === 'qq') return 'qq:' + (song.mid || song.songmid || song.id || (song.name + '|' + song.artist));
...
var defaultHelpers = createPlayerQueueHelpers();
var MineradioPlayerQueue = Object.assign({
  createPlayerQueueHelpers: createPlayerQueueHelpers,
}, defaultHelpers);

if (typeof window !== 'undefined') {
  window.MineradioPlayerQueue = MineradioPlayerQueue;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = MineradioPlayerQueue;
}
